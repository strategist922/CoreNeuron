...
static void alltoalldebug(const char* p, int* s, int* scnt, int* sdispl, int* r, int* rcnt, int* rdispl){
	FILE* f;
	char fname[100];
	sprintf(fname, "debug.%d", nrnmpi_myid);
	f = fopen(fname, "a");
...
//
static void fill_multisend_lists(int use_phase2, int sz, int* r, int*& targets_phase1, int*& targets_phase2) {
...
//printf("%d %d phase1 size=%d\n", nrnmpi_myid, gid, size);
	...
	if (use_phase2) for (Gid2IPS::iterator gid2in_it = gid2in.begin(); gid2in_it != gid2in.end(); ++gid2in_it) {
		InputPreSyn* ps = gid2in_it->second;
...
static int setup_target_lists(int use_phase2, int** r_return) {
	...
	for (Int2TarList::iterator itl_it = gid2tarlist->begin(); itl_it != gid2tarlist->end(); ++itl_it) {
